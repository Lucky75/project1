...
float tmp1 = (float) bi.biHeight*mnoz;
...
{
mnoz = (int) oldHeight/bi.biHeight;

for (int i = 0; i < abs(oldHeight); i+=mnoz)
    {
        
        // iterate over pixels in scanline  // а тут меняем на старый ширину
        for (int j = 0; j < oldWidth; j+=mnoz)
        {
            // temporary storage
            RGBTRIPLE triple;
            for(int k = 0; k < mnoz; k++)
        ...
        // skip over padding in infile
						...
						fseek(inptr, ((oldWidth * 3 + xx)*(mnoz-1)), SEEK_CUR); // и затем переходим в начало строки, через коэффициент строк
        }
