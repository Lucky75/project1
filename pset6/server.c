...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // проверка реквест таргет
            char * x = strstr(line, " /");
            if (x == NULL)
            { 
                printf("не правильное поле %s\n", x);
                error(400);
            }
            ...
            else    
            {
                x1 = strstr(x, " ");
                char queryx[x1-x+1];
                strncpy (query, x, sizeof(queryx)-1);
            ...
            char* path1 = root;
                                                        //printf("Path1: %s \n", path1);
            char* xx = strstr(line, " ")+1;
                                                        //printf ("xx : %s", xx);
            char* yy = strstr(xx,"?");
            if(yy==NULL)
                yy = strstr(xx," ");//-xx-strlen(query);
            //else 
             //   yy--;
                                                    //printf ("YY : %s", yy);    
            int numberok = yy-xx+1;
            
            char path2 [numberok];
            strncpy(path2, xx, numberok);
            path2 [numberok-1] = '\0';
                                                        //printf("Path2: %s\n", path2);
            char* path = strcat(path1, path2);// "TODO";
                                                        printf("Path: %s\n", path);
            if(access (path, 0) )
                error(404);
            ...
            char* extension1 = strstr(path2, ".");//"TODO";
            char* extension = extension1+1;
            printf("Extension: %s\n", extension);
            // dynamic content
            if (strcasecmp("php", extension) == 0)
            {
                ...
                char command[strlen(format) + (sizeof(path) - 2) + (sizeof(query) - 2) + 1];  //strlen
                ...
                if (dprintf(cfd, "Content-Length: %zi\r\n", length) < 0)
                {
                    continue;
                }
